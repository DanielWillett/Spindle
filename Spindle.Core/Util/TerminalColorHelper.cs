...
/// <summary>
/// Handles formatting virtual terminal sequences for changing the color of the terminal.
/// </summary>
public static class TerminalColorHelper
{
    private static StackColorFormatType _libraryColor;
    ...
    /// <summary>
    /// The configuration value for terminal coloring type in <see cref="SpindleLauncher.Configuration"/>.
    /// </summary>
    /// <remarks>The setter should only be used when referencing Spindle as a library, not running it on a server.</remarks>
    /// <exception cref="NotSupportedException">Setter used when Spindle is running on a server.</exception>
    public static StackColorFormatType ColorSetting
    {
        get
        {
            if (!SpindleLauncher.IsActive)
            {
                return _libraryColor;
            }

            if (Enum.TryParse(SpindleLauncher.Configuration["Logging:TerminalColoring"], ignoreCase: true, out StackColorFormatType color)
                && color is StackColorFormatType.ANSIColor or StackColorFormatType.ExtendedANSIColor)
            {
                return color;
            }

            return StackColorFormatType.None;
        }
        set
        {
            if (SpindleLauncher.IsActive)
                throw new NotSupportedException("This value should be set in configuration instead when running on a server.");

            if (value is not StackColorFormatType.ANSIColor and not StackColorFormatType.ExtendedANSIColor and not StackColorFormatType.None)
...
#pragma warning restore CS8500
    ...
    public static unsafe string ConvertRichTextToVirtualTerminalSequences(string str, StackColorFormatType format, int index = 0, int length = -1, RemoveRichTextOptions options = RemoveRichTextOptions.All, int argbForeground = DefaultForeground, int argbBackground = DefaultBackground)
    {
        ...
        int nonDefaults = 1 | 2;
        if (useColor)
            AppendDefaults(ref nonDefaults, ref writeIndex, ref rtn, argbBackground, argbForeground, format);
        ...
        static void AppendDefaults(ref int nonDefaults, ref int writeIndex, ref char[] rtn, int argbBackground, int argbForeground, StackColorFormatType format)
        {
            if ((nonDefaults & 2) != 0)
            {
